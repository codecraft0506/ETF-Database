...
def process_bonds_data(sheet_data, bond_type):
    ...
    # 確保受益人數欄位為數值型
    bonds["受益人數(最新)"] = bonds["受益人數(最新)"].str.replace(",", "").astype(float)
    bonds["受益人數(上月底)"] = bonds["受益人數(上月底)"].str.replace(",", "").astype(float)
    bonds["受益人數(月增)"] = bonds["受益人數(最新)"] - bonds["受益人數(上月底)"]

    # 選取所需欄位
    columns_to_keep = [
        "規模排名", "代號", "名稱", "淨值規模(億)", "淨值規模(億)上個月底",
        "淨值規模(月增)", "受益人數(最新)", "受益人數(上月底)", "受益人數(月增)"
    ]
